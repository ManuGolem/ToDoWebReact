export function ButtonLink({img, name, href,className}){
    if(img){
        ...
        return(
            <a className={`${className} p-3 hover:cursor-pointer hover:text-white hover:scale-105 hover:rounded-2xl hover:bg-violet-500 ease-in duration-200 text-[18px] `} href={href} target="_blank">{name}</a>
        )
    }
}